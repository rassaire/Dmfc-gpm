...
import ShapePoseAndIntesnityModels.{MultiBodyObjectWithIntensity, MultiBodyShapePoseAndIntensityPGA}
...
//    */
  def readShapeAndPoseMultiBodyWithIntensityPGA(file: File): Try[MultiBodyShapePoseAndIntensityPGA[TetrahedralMesh]] = {
   ...
   */
  def writeShapeAndPoseMultiBodyWithIntensityPGA(model: MultiBodyShapePoseAndIntensityPGA[TetrahedralMesh], file: File): Try[Unit] = {
    // currently, we support only the statismo format
...
private object MultibodyIOImpl {
  ...
  def writeStatismoMultiMeshModel(model: MultiBodyShapePoseAndIntensityPGA[TetrahedralMesh],
                                  file: File,
                                  modelPath: String = "/",
                                  statismoVersion: StatismoVersion = v090): Try[Unit] = {

    val discretizedMean = model.gp.mean.data
      .map(v => Array(v.shapeAndIntensityVec.Vec(0) , v.shapeAndIntensityVec.Vec(1), v.shapeAndIntensityVec.Vec(2),v.shapeAndIntensityVec.Intensity, v.poseVec(0), v.poseVec(1), v.poseVec(2)))
      .reduce((acc, s) => acc ++ s)
      .toIndexedSeq
  ...
  private def writeRepresenterStatismov090_MultiBody(h5file: HDF5File,
                                                     group: Group,
                                                     model: MultiBodyShapePoseAndIntensityPGA[TetrahedralMesh],
    ...
    Try {
      ...
      for (i <- 0 to model.gp.domain.objects.size - 1) {
        val cellArray1 = model.gp.domain.objects(i).cells.map(_.ptId1.id) ++
          model.gp.domain.objects(i).cells.map(_.ptId2.id) ++
          model.gp.domain.objects(i).cells.map(_.ptId3.id)++
          model.gp.domain.objects(i).cells.map(_.ptId4.id)
        ...
        for {
          _ <- h5file.writeNDArray[Int](
            ...
            NDArray(IndexedSeq(4, model.gp.domain.objects(i).cells.size), cellArray1.toArray)
          )
          ...
          _ <- h5file.writeNDArray[Float](
            s"$modelPath/representer/intensities" + i,
            NDArray(IndexedSeq(1, model.gp.domain.intensity(i).size), model.gp.domain.intensity(i).toArray.map(_.toFloat))
          )
  ...
  private def writeRepresenterStatismov081_MultiBody(h5file: HDF5File,
    ...
    Try {
      def refAsByteArray(ref: TetrahedralMesh[_3D],
        ...
        for {
          _ <- MeshIO.writeScalarVolumeMeshField(ScalarVolumeMeshField(ref,intensity), tmpfile1)
  ...
  def readStatismoMultiMeshModel(file: File, modelPath: String = "/") = {
    ...
    val modelOrFailure = for {
      ...
      MultiBodyShapePoseAndIntensityPGA(mesh, meanVector, pcaVarianceVector, pcaBasis)
    }
  ...
  private def readStandardMultiMeshFromRepresenterGroup(h5file: HDF5File,
                                                        modelPath: String,
                                                        numberObjects: Int): Try[MultiBodyObjectWithIntensity[_3D, TetrahedralMesh]] = {
    ...
    val mutliobjects = for (i <- 0 to numberObjects - 1) yield {
        ...
        cellArray <- h5file
          ...
          .flatMap(cellArray =>
            if (cellArray.dims(0) != 4)
              Failure(new Exception("the representer cells1 are not tetrahedrons" + i))
            else
              Success(cellArray)
          )
        cellMat = ndIntArrayToIntMatrix(cellArray)
        cells = for (i <- 0 until cellMat.cols)
          yield mesh.TetrahedralCell(PointId(cellMat(0, i)), PointId(cellMat(1, i)), PointId(cellMat(2, i)),PointId(cellMat(3, i)))
        cellArray <- h5file.readNDArray[Int](s"$modelPath/representer/cells" + i)
      } yield mesh.TetrahedralMesh3D(points, TetrahedralList(cells))


      val intensities = for {
        vertArray <- h5file
          .readNDArray[Float](s"$modelPath/representer/intensities" + i)
          .flatMap(vertArray =>
            if (vertArray.dims(0) != 1)
              Failure(new Exception("the representer intensities are not Doubles" + i))
            else
              Success(vertArray)
          )
        vertMat = ndFloatArrayToDoubleMatrix(vertArray)
        intensity = for (i <- 0 until vertMat.cols) yield vertMat(0,i)
      } yield intensity
      ...
      (mesh1.get, intensities.get, rotcenters.get, neutralpts.get)
    }

    Try(
      MultiBodyObjectWithIntensity(mutliobjects.map(ob => ob._1).toList,
                      mutliobjects.map(ob => ob._2).toList,
                      mutliobjects.map(ob => ob._3(0)).toList,
                      mutliobjects.map(ob => ob._4(0)).toList)
    )
  }
  ...
  private def readVTKMultiMeshFromRepresenterGroup(h5file: HDF5File,
    ...
    val mutliobjects = for (i <- 0 to numberObjects - 1) yield {
      ...
      val mesh1 = for {
        ...
        triangleMesh <- MeshIO.readScalarVolumeMeshField[Double](vtkFile)
      } yield triangleMesh
      ...
      (mesh1.get.domain, mesh1.get.data,rotcenters.get, neutralpts.get)
    }
